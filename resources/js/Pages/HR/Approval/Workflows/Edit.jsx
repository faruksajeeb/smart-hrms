...
export default function Edit({
    ...
    const existingLevels = workflow.levels?.length
    ...
    const [levels, setLevels] = useState(existingLevels);

    const initialSelectedCompanies = selectedCompanyBranches
        .map((cb) => cb.company_id)
        .filter((id, index, self) => self.indexOf(id) === index);

    const initialSelectedBranches = selectedCompanyBranches.reduce((acc, cb) => {
        if (cb.branch_id) {
            if (!acc[cb.company_id]) {
                acc[cb.company_id] = [];
            }
            acc[cb.company_id].push(cb.branch_id);
        }
        return acc;
    }, {});
    ...
    const { data, setData, put, processing } = useForm({
        ...
        company_branches: selectedCompanyBranches,
    });
    ...
    function submit(e) {
        e.preventDefault();
        put(route('hr.approval.workflows.update', workflow.id));
    }
    ...
    return (
        <HRLayout
            heading="Edit Approval Workflow"
            subheading="Update approval workflow configuration."
        >
            ...
            <form onSubmit={submit} className="space-y-6">
                                                ...
                                                <div className="mt-3 ml-7 grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-4">
                                                    {companyBranches.map((branch) => (
                                                        ...
                                                        >
                                                            <input
                                                                type="checkbox"
                                                                id={`edit-branch-${company.id}-${branch.id}`}
                                                                checked={selectedBranchIds.includes(branch.id)}
                                                                onChange={() => handleBranchToggle(company.id, branch.id)}
                                                                className="h-4 w-4 rounded border-slate-300"
                                                            />
                                                            <label
                                                                htmlFor={`edit-branch-${company.id}-${branch.id}`}
                ...
                <div className="flex items-center justify-end gap-3">
                    ...
                    >
                        {processing ? 'Saving...' : 'Update Workflow'}
                    </button>
                </div>
            </form>
        </HRLayout>

...
export default function Edit({ policy, detail, errors = {} }) {
    ...
    const { data, setData, put, processing } = useForm({
        ...
        accrual_method: detail.accrual_method ?? 'none',
        monthly_accrual: detail.monthly_accrual ?? '',
        carry_forward_allowed: detail.carry_forward_allowed ?? false,
        maximum_carry_forward: detail.maximum_carry_forward ?? '',
        encashment_allowed: detail.encashment_allowed ?? false,
        maximum_encashment: detail.maximum_encashment ?? '',
        maximum_consecutive_days: detail.maximum_consecutive_days ?? '',
        minimum_days_per_application: detail.minimum_days_per_application ?? 1,
        maximum_days_per_application: detail.maximum_days_per_application ?? '',
        half_day_allowed: detail.half_day_allowed ?? true,
        ...
        include_weekly_off: detail.include_weekly_off ?? false,
        include_holiday: detail.include_holiday ?? false,
        sandwich_rule: detail.sandwich_rule ?? false,
        allow_negative_balance: detail.allow_negative_balance ?? false,
        gender_restriction: detail.gender_restriction ?? 'any',
        marital_status_restriction: detail.marital_status_restriction ?? 'any',
        applicable_after_confirmation: detail.applicable_after_confirmation ?? false,
        status: detail.status ?? 'active',
    });

    function submit(e) {
        e.preventDefault();
        put(route('hr.leave.policies.details.update', [policy.id, detail.id]));
    }

    return (
        <HRLayout
            heading="Edit Policy Rule"
            subheading={`Update rule for ${policy.policy_name}`}
        >
            <Head title="Edit Policy Rule" />
